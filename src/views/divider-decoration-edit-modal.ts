...
// and the live preview renders against the current theme preset. A divider has
...
export interface DividerDecorationEditOptions {
	...
	decoration: DividerDecoration | null;
...
export class DividerDecorationEditModal extends WeWriteModal {
	...
	onOpen(): void {
		...
		contentEl.empty();
		contentEl.addClass('wewrite-divider-deco-modal', 'wewrite-deco-edit-modal');
	...
	private updatePreview(): void {
		...
		const template = this.templateValue.trim() || FALLBACK_TEMPLATE;
	...
	private save(asCopy: boolean): void {
		...
		const name = this.nameValue.trim() || t('deco_edit.default_name_divider');
