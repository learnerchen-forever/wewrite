...
export interface BlockquoteDecorationEditOptions {
...
export class BlockquoteDecorationEditModal extends WeWriteModal {
	...
	onOpen(): void {
		...
		contentEl.empty();
		contentEl.addClass('wewrite-blockquote-deco-modal', 'wewrite-deco-edit-modal');
	...
	private syncParamsFromTemplate(): void {
		for (const m of this.templateValue.matchAll(/\{\{([\w-]+)\}\}/g)) {
			const key = m[1];
			if (key === 'borderImage') continue; // derived from pattern + borderColor
			if (!this.params[key]) {
				this.params[key] = { type: 'text', label: key, default: '' };
			}
		}
	...
	private updatePreview(): void {
		...
		const template = this.templateValue.trim() || '<blockquote style="margin:0;color:#333">{text}</blockquote>';
		const params: Record<string, string> = {};
		for (const [k, v] of Object.entries(this.params)) {
			params[k] = v.default;
		}
		setTrustedHtml(this.previewEl, renderBlockquotePreview(this.options.basePreset, template, params));
	}
	...
	private save(asCopy: boolean): void {
		...
		const name = this.nameValue.trim() || t('deco_edit.default_name_blockquote');
