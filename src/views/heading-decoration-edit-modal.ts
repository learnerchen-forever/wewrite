...
export interface HeadingDecorationEditOptions {
	/** Existing decoration, or null to create a new one. */
	decoration: HeadingDecoration | null;
	...
	onSave: (decoration: HeadingDecoration) => void;
}
...
export class HeadingDecorationEditModal extends WeWriteModal {
	...
	onOpen(): void {
		...
		contentEl.empty();
		contentEl.addClass('wewrite-heading-deco-modal', 'wewrite-deco-edit-modal');
		...
		this.previewEl.addClass('wewrite-deco-preview');
	...
	private updatePreview(): void {
		...
		const template = this.templateValue.trim() || '<h2>{text}</h2>';
		const params: Record<string, string> = {};
		for (const [k, v] of Object.entries(this.params)) {
			params[k] = v.default;
		}
		setTrustedHtml(this.previewEl, renderDecorationPreview(this.options.basePreset, template, params));
	}
	...
	private save(asCopy: boolean): void {
		...
		if (!template.includes('{text}')) {
			new Notice(t('deco_edit.template_needs_text'));
			return;
		}
		this.syncParamsFromTemplate();

		const name = this.nameValue.trim() || t('deco_edit.default_deco_name');
		...
		const decoration: HeadingDecoration = {
			id: baseId,
			name: asCopy ? `${name} ${t('deco_edit.copy_suffix')}` : name,
