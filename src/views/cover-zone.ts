...
export class CoverZone {
  ...
  updateValidityIndicator(accountId: string, hasThumbMediaId: boolean): void {
    if (!accountId || !this.imagePath) {
      this.validityBadge.removeClass('is-shown');
      return;
    }
    this.validityBadge.addClass('is-shown');
    this.validityBadge.toggleClass('is-ready', hasThumbMediaId);
    this.validityBadge.toggleClass('is-pending', !hasThumbMediaId);
    if (hasThumbMediaId) {
  ...
  private buildUI(): void {
    ...
    // Everything but the ratio lives in the .wewrite-cover-zone rule; the
    // ratio is the only per-zone value and stays inline.
  ...
  private updateImageDisplay(): void {
    ...
    if (!this.imagePath) {
      ...
      if (placeholder) placeholder.removeClass('is-hidden');
      return;
    }
    ...
    const placeholder = this.zoneEl.querySelector('.wewrite-zone-placeholder') as HTMLElement;
    if (placeholder) placeholder.addClass('is-hidden');
  }
  ...
  private setupDragDrop(): void {
    this.zoneEl.addEventListener('dragover', (e) => {
      e.preventDefault();
      this.zoneEl.addClass('is-drag-over');
    });

    this.zoneEl.addEventListener('dragleave', () => {
      this.zoneEl.removeClass('is-drag-over');
    });
