...
- **三区封面编辑器。** 一张任意比例的独立封面，外加可选的 **2.35:1 横版 + 1:1 方形**组合，点一次**合成**即得到微信要求的多比例裁剪图。三区都支持双指缩放、拖拽与裁剪框，也都能让 AI 一键生成。
...
LaTeX 数学公式（MathJax SVG）、Mermaid 图表、Excalidraw 手绘、Wiki-link 笔记嵌入，以及 Obsidian / Iconize / Remix 注入笔记的行内图标。
...
未指定尺寸的图片会按自身像素比例填满正文栏，宽截图在手机上也能看清，不必手动改尺寸；自己指定过尺寸的图片（`![[photo.png|200x150]]`）保持原样。表格按内容自适应：窄表格居中而不紧贴左边，宽表格在正文内横向滚动，而不是把列挤窄、把英文单词折成两半。
...
| 功能 | 说明 |
|------|------|
| 校对 | 中英文错别字/语法/标点检测。结果是先看后决的一份清单：每条按严重程度着色（错别字、标点红，语法橙，措辞蓝），点击可跳转，原文里只标记真正改动的字符，建议可先编辑再接受；可逐条接受或忽略，也可一次接受/忽略同类剩余全部，`←`/`→` 切换、`Enter` 接受。有选中文本时校对选中内容，未选中时校对整篇笔记——代码、Frontmatter、链接与 `$公式$` 不会交给模型，因此不会作为「修改建议」返回 |
...
主题 = 带 `wewrite_theme: true` 的 Markdown 笔记（存放在 `wewrite/themes/`）。10 个内置预设——简约 GitHub、经典微信、优雅衬线、纸感长文、网格文档、排版美学、清爽媒体、缤纷强调、温暖日常、暗黑科技——与 10 个可下载主题基于同一套主题系统编写，因此内置主题同样会妆点**每一个**元素（标题、引用、标注、表格、分割线、列表、行内标记、图片、公式、Mermaid、Excalidraw），不再只是旧预设认得的那几项。主题向导一个对话框即可建好主题，主题编辑器可视化定制 100+ 装饰项及其参数。

另有**主题级**设置，一个值贯穿同类全部元素，不必逐条装饰去调：

- **垂直间距** —— 每类块级元素一个外边距（引用、标注、表格、代码块、公式、Mermaid 图、Excalidraw 画布，以及图片），密集的技术笔记因此有自己的节奏，而不必继承各元素自带的间距。
- **图片滑动窗** —— 开启后，没有被空行分隔的图片合并成一行可横向滑动的图片，图片多的文章明显变短；空行仍会另起一行。默认关闭，内置主题与可下载主题各有一半开启。
...
- 2026.09.18 — 发布 2.0.21：38 枚图标按同一套 24 网格与笔画重绘为一个家族，另有 6 处借 Obsidian 内置图标表达 WeWrite 动作的位置改用自绘图标；10 个内置主题重建在与可下载主题同一套系统上；主题新增每类元素的**垂直间距**与**图片滑动窗**开关；未指定尺寸的图片填满正文栏，表格按内容自适应；校对结果改为按严重程度着色的清单，覆盖整篇笔记并跳过代码、Frontmatter 与链接；翻译与图表/公式生成在替换文本前先校验；文生图尺寸按各模型真实约束自动适配；插图可从仓库或设备相册选取；封面合成提速约 20 倍，产物由 48 MB 降为 3 MB PNG。
- 2026.09.15 — 发布 2.0.20：删除账号前先确认；插件在 Obsidian 1.6.6–1.8.6 上恢复加载；设置面板在窄屏或竖屏下改用手机版紧凑布局并保持滚动位置；渲染与发布不再重复读取、重复哈希同一批图片。
