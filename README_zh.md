...
> **关于语言**：Obsidian 社区目录始终读取仓库根目录的 `README.md`，且面向全球用户，
> 因此本仓库以英文版 [README.md](README.md) 为主；本文件是内容完整、同步维护的中文版。
...
入口：**命令面板** + 编辑器右键 **「WeWrite」二级菜单** + 笔记右上角**「⋮」菜单** + 移动端**编辑器工具栏**，四处功能一致。

**插入图片**

| 功能 | 说明 |
|------|------|
| 从仓库插入图片 | 以缩略图网格列出仓库中的全部图片，支持按文件夹筛选与文件名过滤——Obsidian 自带的附件选择器只给出文件名列表，图片一多就无法辨认 |
| 从系统插入图片 | 调用设备自己的图片选择器，手机上即相册。图片会先复制进仓库（写入「新附件的默认位置」设置指定的文件夹），遇到 Obsidian 不支持的格式（如手机直出的 HEIC）自动转为 PNG |

**AI 写作工具**

| 功能 | 说明 |
|------|------|
| 校对 | 中英文错别字/语法/标点检测；Word 式审阅（接受修改 / 忽略 / 上一个 / 下一个），笔记自动定位并高亮错误。有选中文本时校对选中内容，未选中时校对整篇笔记 |
| 同义词 | 结合词语所在句子给出的同义表达，每条附带义项与用法说明，可原地重新查找 |
| 翻译 | 10 种目标语言；代码、链接、公式、标签不会被翻译，译文若丢失了其中一段会在替换前告警 |
| 生成 Mermaid | 描述 → Obsidian 兼容图表。可指定图表类型而不用模型猜；插入前做语法检查（不通过会带错误自动重试一次） |
| 生成公式 | 描述 → LaTeX，可选块级或行内；插入前用 Obsidian 自己的 MathJax 真编译校验 |
...
> 教程目前为中文；英文功能总览见 [README.md](README.md)。

---

## 更新历史

> 每个版本具体改了什么，见 [CHANGELOG.md](CHANGELOG.md)。插件升级后会自动弹出「更新说明」，
> 也可以随时用命令面板里的 **「查看 WeWrite 更新内容」** 再次打开。
...
- 2026.09.13 — 发布 2.0.19：全部命令补齐图标，可在手机编辑器工具栏直接调用；引用样式回到各主题自己的强调色，并修复嵌套引用与未装饰引用的排版；主题向导的引用预设改用当前装饰 Schema；合并外部 PR [#28](https://github.com/learnerchen-forever/wewrite/pull/28)（Synology `app://` 图片路径、列表内行内公式、复制到微信前先上传图片）；新增「更新说明」弹窗；发布说明改由 `CHANGELOG.md` 统一生成。
- 2026.09.07 — 发布 2.0.18。
